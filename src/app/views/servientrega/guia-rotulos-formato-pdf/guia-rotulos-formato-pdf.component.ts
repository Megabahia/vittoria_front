...
export class GuiaRotulosFormatoPdfComponent implements OnInit {
  ...
  generarGuia(): void {
    ...
    this.guiasService.generarRotulosFormatoPDF(this.reactiveGuideForm.value.num_guia).subscribe((info) => {
      this.data = info;
  ...
  getImage(fileBase64: string): void {
    ...
    downloadLink.download = 'GENERACION_DE_ROTULOS_FORMATO_PDF.pdf';
